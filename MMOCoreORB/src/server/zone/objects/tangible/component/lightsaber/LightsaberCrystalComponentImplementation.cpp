...
void LightsaberCrystalComponentImplementation::validateCrystalStats() {
	...
	const float yellowModifier = lootManager->getYellowModifier();
	const float exceptionalModifier = lootManager->getExceptionalModifier();
	const float legendaryModifier = lootManager->getLegendaryModifier();
	if (crystalExcMod > 1 && crystalExcMod <= yellowModifier){
		damage += 10;
		woundChance += 1;
		floatForceCost -= 0.50;
	} else if (crystalExcMod > yellowModifier && crystalExcMod <= exceptionalModifier){
		damage += 15;
		woundChance += 2;
		floatForceCost -= 0.75;
	} else if (crystalExcMod > exceptionalModifier && crystalExcMod <= legendaryModifier){
		damage += 20;
		woundChance += 3;
		floatForceCost -= 1.25;
	}
	...
	if (object->hasSkill("force_title_jedi_rank_01") || player->isPrivileged()) {
		...
		if (getColor() != 31) {
			...
			if (ownerID != 0 || player->isPrivileged()) {
				...
				//alm->insertAttribute("execMod", crystalExcMod);
