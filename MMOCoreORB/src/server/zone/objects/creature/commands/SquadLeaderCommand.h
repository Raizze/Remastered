...
public:
	...
	static bool isValidGroupAbilityTarget(CreatureObject* leader, CreatureObject* target, bool allowPet) {
		...
		if (ghost == nullptr || (ghost->hasBhTef() && !ghost->hasPvpTef()))
			return false;
		...
		if (leaderFaction == 0) {
			if (targetFaction != 0 && targetStatus == FactionStatus::OVERT)
				return false;
		} else if (targetFaction != 0) {
			if (leaderFaction != targetFaction && targetStatus == FactionStatus::OVERT )
				return false;
